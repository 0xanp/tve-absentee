...
class AppFrame(wx.Frame):
    ...
    def load_options(self, start_date, end_date):
        ...
        date_picker.clear()
        date_picker.send_keys(start_date.strftime("%d/%m/%Y"), Keys.ENTER)
        # end date picker
        date_picker = WebDriverWait(driver, 2).until(
            EC.element_to_be_clickable((By.XPATH,'//*[@id="denngay"]')))
        date_picker.clear()
        date_picker.send_keys(end_date.strftime("%d/%m/%Y"), Keys.ENTER)
    ...
    def OnOkClick(self, evt):
        ...
        driver, df = self.load_options(self.start_date, self.end_date)
        print(df)
        percent = 0
        course_dicts = {}
        """
